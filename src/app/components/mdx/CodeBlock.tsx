...
export function Codeblock(
  ...
  return (
    <>
      ...
      >
        <TbCopy
          className={`absolute h-6 w-6 p-0 transition-all ${
            copied ? "scale-0" : ""
          }`}
        />
        <FaCheck
          className={`absolute h-6 w-6 scale-0 p-1 transition-all ${
            copied ? "scale-100" : ""
          }`}
        />
      </div>
      <pre
        ...
        className="relative my-4 overflow-x-scroll rounded-lg border border-zinc-400  p-4 font-mono text-sm font-semibold bg-stone-700 text-zinc-300"
      >
        {props.children}
      </pre>
    </>
